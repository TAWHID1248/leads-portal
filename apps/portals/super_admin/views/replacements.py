...
def replacements_view(request):
    ...
    return render(request, 'super_admin/replacements/list.html', {
        'page_title': 'Replacement queue',
        'pending': pending,
        'decided': decided,
    })
...
def deny_replacement_view(request, pk):
    ...
    ActivityLog.objects.create(
        ...
        entity_type='ReplacementRequest', entity_id=str(rr.id),
        metadata={'lead_id': rr.lead_id, 'note': note},
    )
